...
using CoursesAPI.Services.Pagination;
...
namespace CoursesAPI.Services.Services
{
	public class CoursesServiceProvider
	{
		...
		/// <returns></returns>
		public Envelope<CourseInstanceDTO> GetCourseInstancesBySemester(string semester = null, int page = 1, string language = "is")
		{
            int PAGESIZE = 10;
            page = page - 1; 
			...
			var courses = (from c in _courseInstances.All()
				...
				{
					Name               = (language == "en") ? ct.NameEN : ct.Name, // TODO: select the name based on the language requested (if any)
					TemplateID         = ct.CourseID,
					CourseInstanceID   = c.ID,
					MainTeacher        = "" // Hint: it should not always return an empty string!
				}).OrderBy(c => c.CourseInstanceID).Skip(page * PAGESIZE).Take(PAGESIZE).ToList();

            //Get the paging information:
            int totalNumberOfItems = (from c in _courseInstances.All()
                                      join ct in _courseTemplates.All() on c.CourseID equals ct.CourseID
                                      where c.SemesterID == semester
                                      select ct.CourseID).Count();


            Envelope<CourseInstanceDTO> envelope = new Envelope<CourseInstanceDTO>(courses, PAGESIZE, page + 1, totalNumberOfItems);
